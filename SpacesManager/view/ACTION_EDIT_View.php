		...
		<div class="container">
            ...
            <?=htmlentities($strings["Do you want to change something?"])?>
					...
					<div class="col-lg-12 center-block-content">
						<form  method="POST" action="ACTION_Controller.php?action=<?= $strings['Edit']?>&accion=<?=$this->values['sm_idAction']?>" onkeyup="validateAction(this)">
							<div id="group-form">

								<div class="input-container">
									<span class="input-group-text fa fa-users"></span>
									<input type="text" id="nameAction" name="nameAction" value="<?=$this->values['sm_nameAction']?>" onkeyup="checkText(this)" required/>
									<label for="nameAction"><?= $strings['What action is it?']?></label>
								</div>

								<div class="input-container">
									<span class="input-group-text fa fa-reorder"></span>
									<input type="text" id="descripAction" name="descripAction" value="<?=$this->values['sm_descripAction']?>" onkeyup="checkText(this)" required/>
